...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        ...
        for i in range(len(valid_words)):
            pair.append([diff_function(user_word,valid_words[i],limit),i])
        if min(pair,key=lambda item:item[0])[0] <= limit:
            return valid_words[min(pair,key=lambda item:item[0])[1]]
        return user_word
...
def time_per_word(times_per_player, words):
    ...
    for i in times_per_player:
        ...
        for word in range(len(words)):
            player_time_list.append(i[index + 1] - i[index])
            index = index + 1
        times_list.append(player_time_list)
...
def fastest_words(game):
    ...
    # BEGIN PROBLEM 10
    fastest_list = [[] for player in player_indices]
    for word in word_indices:
        index = 0
        player_time = [time(game, player, word) for player in player_indices]
        for i in player_time:
            if i == min(player_time):
                fastest_index = index
                break
            index = index + 1
        fastest_list[fastest_index].append(all_words(game)[word])
    return fastest_list
    # END PROBLEM 10
